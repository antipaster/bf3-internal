#include "FB SDK/CharacterPhysicsEntityState.h"
...
void DrawCircle(fb::Vec3 center, float radius, fb::Color32 color, int segments = 64)
{
    if (segments < 3) segments = 3;
    
    fb::Vec3 prevPoint, currentPoint;
    float angleStep = 2.0f * 3.14159f / segments;
    
    for (int i = 0; i <= segments; i++) {
        float angle = i * angleStep;
        currentPoint.x = center.x + radius * cos(angle);
        currentPoint.y = center.y + radius * sin(angle);
        currentPoint.z = center.z;
        
        if (i > 0) {
            Draw2dLine(prevPoint, currentPoint, color);
        }
